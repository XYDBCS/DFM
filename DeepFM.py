"""
Tensorflow implementation of DFM
"""
...
from mine_function import read_txt1, get_lapl_matrix
...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # ---------- Deep component ----------
            self.y_deep = tf.reshape(self.embeddings, shape=[-1, self.field_size * self.embedding_size]) # None * (F*K)， flatten the
            ...
            lap_matx_m = get_lapl_matrix(self.sim_m)
            lap_matx_d = get_lapl_matrix(self.sim_d)
            ...
            self.l_loss = tf.add(tf.multiply(self.m_loss, tf.constant([0.03], tf.float32)), tf.multiply(self.d_loss, tf.constant([0.03], tf.float32)))
